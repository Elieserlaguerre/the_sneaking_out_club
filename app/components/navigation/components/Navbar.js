		...
		<div className="bg-gray-900">
			<header className="relative isolate z-50">
				...
				<nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-end lg:justify-start p-6 lg:px-8 gap-4">
					<div className={classNames("flex lg:hidden")}>
					...
					{navList?.map((section, i) => {
						if (section.title) {
							return (
								<Link key={i} className={classNames(path === section.href ? "ring-1 ring-white rounded-sm" : null, "hidden lg:flex items-center gap-x-1 text-sm font-semibold leading-6 text-white capitalize px-2.5 py-1 hover:ring hover:ring-white hover:rounded-sm")} href={section.href}>
									{section.title}
								</Link>
							);
						}
							...
							<div className="-my-6 divide-y divide-gray-500/10">
								<div className="space-y-2 py-6">
									{navList?.map((section, i) => {
										if (section.title) {
											return (
												<Link key={i} onClick={() => setMobileMenuOpen(false)} className={classNames(path === section.href ? "ring-1 ring-black" : null, "group flex w-full items-center justify-between rounded-lg py-2 px-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50 capitalize")} href={section.href}>
													{section.title}
												</Link>
											);
										}
