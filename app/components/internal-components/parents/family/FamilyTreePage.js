...
export default function FamilyTreePage() {
	...
	useEffect(() => {
		if (getFamilyTreesResults.isError) {
			...
			if (results?.actions === "my family tree") {
				setMyFamilyTree(results?.familyTree);
			} else {
	...
	useEffect(() => {
		if (user?._id && filters.actions !== "search family trees") getFamilyTrees({ userId: user._id, page, limit, filter: JSON.stringify(filters) });
		else if (filters.actions === "search family trees") {
			handleSearchModal();
		}
		...
		<div>
			<div className="mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8 min-h-screen flex flex-col">
				...
				<div className="hidden">
					...
					<FamilyTreeSearch open={openSearchModal} closingFunction={closeSearchModal} updateFunction={updateSearchResults} page={page} limit={limit} filters={filters} />
				</div>
			</div>
		</div>
	);
