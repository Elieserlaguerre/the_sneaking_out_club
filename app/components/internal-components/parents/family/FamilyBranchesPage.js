...
export default function FamilyBranchesPage() {
	...
	const handleDrawerEditMode = (selection) => {
		setEditMode(true);
		setOpenDrawer(true);
	...
	useEffect(() => {
		if (getHouseholdsResults.isError) {
			...
			if (results?.actions === "my family") {
				setMyHousehold(results?.family);
			} else {
				setFamilies(results?.families ?? []);
					...
					{filters.actions === "my family" ? (
						<div className="size-full min-h-screen">
							{!myHousehold ? (
								...
								<FamilyProfile family={myHousehold} editFunction={handleDrawerEditMode} />
							)}
						...
						<Fragment>
							<div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 bg-gray-200 gap-2">{getHouseholdsResults.isFetching ? "" : families.length === 0 ? <EmptyFamilyMembers /> : families?.map((member) => <FamilyMemberCard key={member?._id} member={member} editFunction={handleDrawerEditMode} />)}</div>
