...
import { useDeleteAncestorMutation, useManageAncestorMembershipMutation } from "@/app/lib/redux/data-fetching/parents-api";
import toast from "react-hot-toast";

export default function AncestorCard({ ancestor, tree }) {
	...
	const [manageAncestorMembership, manageAncestorMembershipResults] = useManageAncestorMembershipMutation();

	useEffect(() => {
		if (manageAncestorMembershipResults.isError) {
			const message = typeof manageAncestorMembershipResults.error === "string" ? manageAncestorMembershipResults.error : manageAncestorMembershipResults.error.message;
			toast.error(message);
		} else if (manageAncestorMembershipResults.isSuccess) {
			toast.success(manageAncestorMembershipResults.data.message);
		}
	}, [manageAncestorMembershipResults.isLoading, manageAncestorMembershipResults.isSuccess, manageAncestorMembershipResults.isError]);

	const handleFamilyMembership = (action) => {
		switch (action) {
			case "add":
				manageAncestorMembership({ action, ancestorId: ancestor._id, treeId: tree._id });
				break;
			case "remove":
				manageAncestorMembership({ action, ancestorId: ancestor._id, treeId: tree._id });
				break;
	...
	const [deleteAncestor, deleteAncestorResults] = useDeleteAncestorMutation();

	useEffect(() => {
		if (deleteAncestorResults.isError) {
			const message = typeof deleteAncestorResults.error === "string" ? deleteAncestorResults.error : deleteAncestorResults.error.message;
			toast.error(message);
		} else if (deleteAncestorResults.isSuccess) {
			toast.success(deleteAncestorResults.data.message);
		}
	}, [deleteAncestorResults.isLoading, deleteAncestorResults.isSuccess, deleteAncestorResults.isError]);

	const handleAncestorDeletion = () => {
		if (ancestor.creator !== user._id) return toast.error("You are not authorized to delete this user.");
	...
	return (
		<div className="rounded-lg bg-white text-center p-8 shadow-md shadow-gray-400 dark:divide-white/10 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10 border border-gray-400 aspect-square group relative">
			<div className="absolute top-0 right-0 left-0 px-4 py-5 sm:px-6 hidden group-hover:flex justify-end items-center">
				<Menu as="div" className="relative">
					...
					<MenuItems transition className="absolute right-0 z-10 mt-2 -mr-1 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-75 data-leave:ease-in grid grid-cols-1 gap-1 p-1">
						...
						<MenuItem>
							<button onClick={handleAncestorDeletion} className={classNames(buttonVariants({ variant: "destructiveBtn" }))}>
								delete
							</button>
						</MenuItem>
					</MenuItems>
			...
			<div className="p-2 shink">
				<dl className="size-full flex flex-col justify-evenly items-center text-sm">
					<div className="w-full mt-3">
						...
						<dd className="capitalize">{ancestor?.relation?.map((family) => (family.member === user._id ? family.role : null)).filter(Boolean)}</dd>
					</div>
					<div className="w-full mt-3">
			...
			{ancestor.creator.toString() === user._id && (
