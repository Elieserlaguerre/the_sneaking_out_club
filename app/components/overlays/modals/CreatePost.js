		...
		<div className="divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow-sm dark:divide-white/10 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10 shrink-0">
			<div className="p-4  sm:px-6 flex justify-between items-center gap-x-6">
				...
				<div className="flex-auto">
					<input onSelect={handleModalOpenning} type="text" name="post" id="post" className="bg-gray-200 rounded-full w-full px-2.5 py-3.5 h-full" placeholder={`What's on your mind, ${user?.lastName}?`} />
				</div>
							...
							<DialogPanel transition className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg sm:p-6 data-closed:sm:translate-y-0 data-closed:sm:scale-95 dark:bg-gray-800 dark:outline dark:-outline-offset-1 dark:outline-white/10">
								<form onSubmit={handleSubmit} className="flex flex-col">
									<div>
										...
										<div className="mt-3 sm:mt-5 divide-y divide-gray-300 overflow-hidden rounded-lg bg-white shadow-sm dark:divide-white/10 dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10">
											...
											<div className="px-4 py-5 sm:p-6">
												{formContent.type === "text" ? (
													<input onChange={handleChanges} type="text" name="caption" id="caption" className="bg-gray-200 rounded-full w-full px-2.5 py-3.5 h-full" placeholder={`What's on your mind, ${user?.lastName}?`} />
												) : (
													<div className="divide-y divide-gray-200 w-full h-28 flex flex-col">
