...
import { useLazyGetFamilyTreeLeadershipCandidatesQuery, useLazyGetRootFamilyQuery } from "@/app/lib/redux/data-fetching/parents-api";
...
import FamilyLeaderCandidateCard from "../../cards/FamilyLeaderCandidateCard";

export default function ManageFamilyTreeLeadership({ open, closingFunction, tree }) {
	...
	const [getLeadershipCandidates, getLeadershipCandidatesResults] = useLazyGetFamilyTreeLeadershipCandidatesQuery();

	console.log("query", useLazyGetFamilyTreeLeadershipCandidatesQuery);

	useEffect(() => {
		if (getLeadershipCandidatesResults.isError) {
			const message = typeof getLeadershipCandidatesResults.error === "string" ? getLeadershipCandidatesResults.error : getLeadershipCandidatesResults.error.message;
			toast.error(message);
		} else if (getLeadershipCandidatesResults.isSuccess) {
			// toast.success(getLeadershipCandidatesResults.data.message);

			const { results } = getLeadershipCandidatesResults.data;

			setTotalPages(results?.totalPages ?? 0);
			setCandidates(results?.candidates ?? []);

			if (results?.candidates?.length === 0) closingFunction();
		}
	}, [getLeadershipCandidatesResults.isFetching, getLeadershipCandidatesResults.isSuccess, getLeadershipCandidatesResults.isError]);
	...
	useEffect(() => {
		if (tree) getLeadershipCandidates({ treeId: tree._id, page, limit, filters: JSON.stringify(filters) });
	}, [user, page, limit, filters, tree]);
							...
							<div className="text-center">
								{candidates?.length === 0 ? (
									<div className="text-center border border-gray-400 w-full py-4 flex flex-col justify-center items-center">
										...
										<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by adding new family members to the family tree.</p>
									</div>
								) : (
									<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 py-4 gap-2">
										{candidates?.map((member) => (
											<FamilyLeaderCandidateCard member={member} tree={tree} key={member._id} />
										))}
									</div>
								)}
							</div>
