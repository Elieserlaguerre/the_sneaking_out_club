...
import { useLazyGetRootFamilyQuery } from "@/app/lib/redux/data-fetching/parents-api";
...
export default function AddRootFamilyMembers({ open, closingFunction, tree }) {
	...
	const [getRootFamily, getRootFamilyResults] = useLazyGetRootFamilyQuery();

	useEffect(() => {
		if (getRootFamilyResults.isError) {
			const message = typeof getRootFamilyResults.error === "string" ? getRootFamilyResults.error : getRootFamilyResults.error.message;
			toast.error(message);
		} else if (getRootFamilyResults.isSuccess) {
			// toast.success(getRootFamilyResults.data.message);

			const { results } = getRootFamilyResults.data;

			setTotalPages(results?.totalPages ?? 0);
			setRootFamily(results?.rootFamily ?? []);

			if (results?.rootFamily?.length === 0) closingFunction();
		}
	}, [getRootFamilyResults.isFetching, getRootFamilyResults.isSuccess, getRootFamilyResults.isError]);
	...
	useEffect(() => {
		if (user) getRootFamily({ parentId: user._id, page, limit, filters: JSON.stringify(filters), treeId: tree._id });
						...
						<div>
							<div className="bg-linear-to-br from-blue-500 to-blue-700 p-4 rounded-t-md relative">
								...
								<DialogTitle as="h3" className="text-base font-semibold text-white text-center capitalize">
									add root family members
								</DialogTitle>
							</div>
							<div className="text-center">
								{rootFamily?.length === 0 ? (
									<div className="text-center border border-gray-400 w-full py-4 flex flex-col justify-center items-center">
										<h3 className="mt-2 text-sm font-semibold text-gray-900 dark:text-white capitalize">No rootFamily to display</h3>
										<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by adding a new root family member.</p>
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 py-4 gap-2">
										{rootFamily?.map((member) => (
											<RootMemberCard member={member} tree={tree} key={member._id} />
										))}
									</div>
								)}
							</div>
